...
if(isset($_POST['submit']))
{
    ...
    if(preg_match('/^\d{10}$/', $enrollment) && substr($enrollment,2, 4) === '1051') {
		...
		//echo "<span style='color:red'>Enrollment No. is available for Registration .</span>";
 		//echo "<script>$('#submit').prop('disabled',false);</script>";
    } else {
        $msg="Invalid enrollment number. Registration failed.";
		//echo "<span style='color:green'>Enrollment No. is not available for Registration .</span>";
  ...
  <body>
	...
	<hr />
		      <form class="form-login" method="post">
		        ...
		        <p align="center" style="padding-top: 10%; color: blue ">
		        	<?php if($msg){
echo htmlentities($msg);
				...
				<select name="applicant type" class="form-control" required="required">
				    <option value="" disabled selected>Applicant Type</option>
				...
				<input type="tel" class="form-control" placeholder="Enrollment No." id="enroll" oninput="validateEnrollment()" name="enroll" required="required">
<span id="enrollment-error" style="font-size:12px; color:red;"></span>
    ...
    <script>
        $.backstretch("assets/img/bg2.jpg", {speed: 500});
    </script>
